...
class GroupCenterCropFM(object):
    def __init__(self, size):
        self._size = size

    def __call__(self, img_and_mv):
        img_group, mv_group  = img_and_mv
        h, w, _ = img_group[0].shape
        hs = (h - self._size) // 2
        ws = (w - self._size) // 2
        ret_img = [img[hs:hs+self._size, ws:ws+self._size] for img in img_group]
        ret_mv = [mv[hs:hs+self._size, ws:ws+self._size] for mv in mv_group]
        return (ret_img,ret_mv)




class GroupScaleFM(object):
    def __init__(self, size):
        self._size = (size, size)

    def __call__(self, img_and_mv):
        img_group, mv_group  = img_and_mv
        ret_img = [cv2.resize(img, self._size, cv2.INTER_LINEAR) for img in img_group]
        ret_mv =  [resize_mv(mv, self._size, cv2.INTER_LINEAR) for mv in mv_group]
        return (ret_img, ret_mv)


class GroupRandomHorizontalFlipFM(object):
    def __init__(self):
        super(GroupRandomHorizontalFlipFM,self).__init__()

    def __call__(self, img_and_mv):
        img_group, mv_group = img_and_mv
        if random.random() < 0.5:
            ret_img = [img[:, ::-1, :].astype(np.int32) for img in img_group]
            ret_mv = [mv[:, ::-1, :].astype(np.int32) for mv in mv_group]

            for i in range(len(ret_mv)):
                ret_mv[i] -= 128
                ret_mv[i][..., 0] *= (-1)
                ret_mv[i] += 128
            ...
            return (img_group,mv_group)


class GroupMultiScaleCropFM(object):
    def __init__(self, input_size, scales=None, max_distort=1, fix_crop=False, more_fix_crop=True):
        self.scales = scales if scales is not None else [1, .875, .75, .66]
        self.max_distort = max_distort
        self.fix_crop = fix_crop
    ...
    def __call__(self, img_and_mv):
        img_group, mv_group = img_and_mv
        im_size = img_group[0].shape
        mv_size = mv_group[0].shape 

        crop_w, crop_h, offset_w, offset_h = self._sample_crop_size(im_size)

        crop_img_group = [img[offset_w:offset_w + crop_w, offset_h:offset_h + crop_h] for img in img_group]
        crop_mv_group = [mv[offset_w:offset_w + crop_w, offset_h:offset_h + crop_h] for mv in mv_group]
        ...
        ret_mv_group = [resize_mv(mv, (self.input_size[0], self.input_size[1]), cv2.INTER_LINEAR)
                            for mv in crop_mv_group]
        return (ret_img_group,ret_mv_group)
...
class GroupOverSampleFM(object):
    def __init__(self, crop_size, scale_size=None):
        self.crop_size = crop_size if not isinstance(crop_size, int) else (crop_size, crop_size)

        if scale_size is not None:
            self.scale_worker = GroupScaleFM(scale_size)
        else:
            self.scale_worker = None
    ...
    def __call__(self, img_and_mv):
        ...
        if self.scale_worker is not None:
            img_and_mv = self.scale_worker(img_and_mv)
        img_group, mv_group = img_and_mv
        image_w, image_h, _ = img_group[0].shape
        crop_w, crop_h = self.crop_size

        offsets = GroupMultiScaleCrop.fill_fix_offset(False, image_w, image_h, crop_w, crop_h)
        oversample_img_group = list()
        oversample_mv_group = list()
        for o_w, o_h in offsets:
            for img in img_group:
                crop = img[o_w:o_w+crop_w, o_h:o_h+crop_h]
                oversample_img_group.append(crop)

                flip_crop = crop[:, ::-1, :].astype(np.int32)
                oversample_img_group.append(flip_crop)
                
            for mv in mv_group:
                crop = mv[o_w:o_w+crop_w, o_h:o_h+crop_h]
                oversample_mv_group.append(crop)

                flip_crop = crop[:, ::-1, :].astype(np.int32)
                flip_crop -= 128
                flip_crop[..., 0] *= (-1)
                flip_crop += 128
                oversample_mv_group.append(flip_crop)
                
        return (oversample_img_group, oversample_mv_group)